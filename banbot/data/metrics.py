...
def _calc_drawdowns(profits: List[float], init_balance: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    返回：cumulative, highs, drawdown, drawdown_pct
    '''
    ...
    if init_balance:
        ...
        drawdown_pct = (max_balance - cumulative_balance) / max_balance
    else:
        drawdown_pct = (highs - cumulative) / highs
    return cumulative, highs, drawdown, drawdown_pct
...
def calc_max_drawdown(profits: List[float], init_balance: float = 0, relative=False):
    ...
    '''
    cumulative, highs, drawdown, drawdown_pct = _calc_drawdowns(profits, init_balance)
    try:
        idxmin = np.argmax(drawdown_pct) if relative else np.argmin(drawdown)
    except Exception:
        return 0, -1, -1, 0, 0, 0
    idxmax = np.argmax(highs[:idxmin + 1])
    high_val = cumulative[idxmax]
    low_val = cumulative[idxmin]
    max_drawdown_rel = drawdown_pct[idxmin]
